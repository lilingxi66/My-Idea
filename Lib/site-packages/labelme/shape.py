...
DEFAULT_HOVER_LINE_COLOR = QtGui.QColor(255, 210, 0, 220)
DEFAULT_HOVER_FILL_COLOR = QtGui.QColor(255, 210, 0, 60)
...
class Shape(object):
    ...
    def setClosed(self, closed):
        self.closed = closed  # 标记多边形是否闭合
    
    def isClosed(self):
        # 修复原有isClosed逻辑，适配手动标记的closed状态
    ...
    def paint(self, painter):
        if self.points:
            ...
            if self.hovered_edge_index is not None and len(self.points) >= 2:
                hover_pen = QtGui.QPen(self.hover_line_color)
                hover_pen.setWidth(max(2, int(round(3.0 / self.scale))))
                painter.setPen(hover_pen)
                start_point = self.points[self.hovered_edge_index - 1]
                end_point = self.points[self.hovered_edge_index]
